    ...
    function handlePosSearch(val) {
        ...
        if (filtered.length > 0) {
            ...
            grid.innerHTML = filtered.map(p => {
                ...
                const isLow = typeof utils.inventoryIsLowStock === 'function' ? utils.inventoryIsLowStock(p) : stock <= 5;
                const stockTone = stock <= 0 ? 'vc884-stock-out' : (isLow ? 'vc884-stock-low' : 'vc884-stock-ok');
                return `<div class="py-3 px-2 border-b border-border-subtle last:border-0"><div class="flex items-center gap-2 mb-2 min-w-0"><h4 class="font-black text-sm text-on-surface min-w-0 flex-1 truncate">${escapeHTML(p.name)}</h4><span class="text-[8px] font-black uppercase bg-primary-container text-primary px-2 py-0.5 rounded-full shrink-0">${escapeHTML(p.category || 'General')}</span><span class="vc884-stock-pill ${stockTone}">Stock: ${stock.toLocaleString()}</span></div><div class="flex gap-2"><button onclick="addToCart(${jsArg(p.id)}, 'piece')" class="flex-1 bg-surface-container py-2.5 px-3 text-left rounded-xl active-scale"><p class="text-[8px] uppercase font-bold opacity-50">Piece</p><p class="font-black text-xs text-primary">${formatCurrency(p.price)}</p></button>${p.packPrice ? `<button onclick="addToCart(${jsArg(p.id)}, 'pack')" class="flex-1 bg-secondary/5 py-2.5 px-3 text-left rounded-xl active-scale"><p class="text-[8px] uppercase font-bold text-secondary">Pack (${escapeHTML(p.packSize)})</p><p class="font-black text-xs text-secondary">${formatCurrency(p.packPrice)}</p></button>` : ''}</div></div>`;
            }).join('');
