// --- Firebase Configuration ---
    ...
    // v5.6.45: Critical Fix - Enable Firestore Offline Persistence explicitly
    ...
    function setupRealTimeSync() {
        ...
        // v5.6.45: Offline Verified Snapshots
        ...
        businessDaysUnsubscribe = db.collection('businessDays').onSnapshot({ includeMetadataChanges: true }, (snapshot) => {
            ...
            // v5.6.45: Preserve local business-day records while Firestore catches up.
    ...
    function showSyncInfo() {
        ...
        alert(`Cloud Connection Status: ${status}\n\n${msg}\n\nSync Engine: Robust Direct-Sync v5.6.45`);
    }
    ...
    function queueTransaction(transaction) {
        ...
        // v5.6.45 CORE BUSINESS DAY ATTACHMENT
    ...
    // v5.6.45 UI Polish Fix: keep active nav synced on mobile/tablet
...
function switchScreen(id) {
    ...
    function confirmSale() {
        ...
        // v5.6.45: Ensure every new transaction is linked to a business day before syncing.
    ...
    // v5.6.45 Inventory PIN navigation polish
    ...
    // v5.6.45 Cash amount selection polish
    ...
    // v5.6.45 Business Dashboard calculations
    ...
    // v5.6.45 Store Closing Preview Modal
...
function getClosingCounts(transactions) {
    ...
    // v5.6.45 Reporting Fallback: never hide real transactions because businessDayId is missing
    ...
    // v5.6.45 Core Business Day Attachment + Reporting Repair
    ...
    // v5.6.45 Delete Transaction Modal Fix
    ...
    // v5.6.45 Business Day Manager - core architecture
    ...
    // v5.6.45 Business Day Date-Scope Fix
    ...
    // v5.6.45 Dashboard wording and credit clarity polish
    ...
    // v5.6.45 Transaction Integrity Layer
    ...
    // v5.6.45 Authoritative Realtime Reporting Engine
    ...
    // v5.6.45 Credit/Settlement Void Guidance + Color Coding
    ...
    // v5.6.45 Final UI Override: clickable Insight cards + real Business month label
    ...
    // v5.6.45 Cross-device Recent Activities Fix
    ...
    // v5.6.45 Cross-device Business Day Card Fix
    ...
    // v5.6.45 Brand Header Controller
    ...
    // v5.6.45 Premium Header Text Normalizer
    ...
    // v5.6.45 Ultra Compact Header Date Line
    ...
    // v5.6.45 Stable Header Controller
    ...
    // v5.6.45: Retire persistent deleted-transaction caches.
...
// v5.6.45 Ledger polish: readable transaction cards, quick filters, safer status labels.
(function(){
...
// v5.6.45 Sync safety: auto retry pending work and stop UI repair write loops.
(function(){
...
// v5.6.45 Cross-device reconcile: keep realtime, plus safe focus/online cloud refresh.
(function(){
...
// v5.6.45 Stability + UI: collision-proof transaction IDs, ledger date groups, insight debounce, faster PIN.
(function(){
...
// v5.6.45 UI responsiveness: stable Insights, faster Stock PIN, auto-focus searches.
(function(){
...
// v5.6.45 Stability + sync polish: single-owner Insights, restored credit pay-full,
...
(function(){
    ...
    // Keep search focus helpful, but suppress the v5.6.45 every-button autofocus
...
// v5.6.45 Device UI + sync fix: remove automatic keyboard focus, make checkout
...
(function(){
    ...
    async function refreshCloud(reason, includeInventory) {
        ...
        try {
            ...
            console.warn('v5.6.45 cloud freshness failed', reason, e);
            return false;
        } finally {
            refreshing = false;
        }
...
// v5.6.45 hard UI/sync fix: no programmatic focus, cloned checkout input reset,
...
(function(){
    ...
    async function pullCloud(reason, includeInventory){
        ...
        try {
            ...
            console.warn('v5.6.45 cloud pull failed', reason, e);
            return false;
        } finally {
            cloudBusy = false;
        }
...
// v5.6.45 Safe boot: always show POS first, then run optional startup work in guards.
(function(){
    if (window.__vc5641SafeBoot) return;
    window.__vc5641SafeBoot = true;

    function vc5641Log(label, err) {
        try { console.warn('[VC v5.6.45 safe boot]', label, err); } catch (_) {}
    }
    ...
    function vc5641ShowPosFallback() {
        vc5641Call('show POS fallback', function(){
            document.querySelectorAll('.screen-transition').forEach(function(screen){
                if (screen && screen.id === 'screen-pos') screen.classList.remove('hidden');
                else if (screen) screen.classList.add('hidden');
            });
            document.querySelectorAll('.nav-btn').forEach(function(btn){
                const isPos = btn && btn.getAttribute && btn.getAttribute('onclick') && btn.getAttribute('onclick').includes("switchScreen('pos')");
                btn.classList.toggle('active', !!isPos);
            });
        });
    }

    function vc5641LightRender() {
        vc5641ShowPosFallback();
        vc5641Call('applyUIPolish', function(){ return typeof applyUIPolish === 'function' && applyUIPolish(); });
        vc5641Call('renderFavorites', function(){ return typeof renderFavorites === 'function' && renderFavorites(); });
        vc5641Call('updateCartUI', function(){ return typeof updateCartUI === 'function' && updateCartUI(); });
        vc5641Call('updateSyncUI', function(){ return typeof updateSyncUI === 'function' && updateSyncUI(); });
    }

    function vc5641FullBoot() {
        if (window.__vc5641FullBootRan) return;
        window.__vc5641FullBootRan = true;

        vc5641LightRender();
        setTimeout(function(){ vc5641Call('setupRealTimeSync', function(){ return typeof setupRealTimeSync === 'function' && setupRealTimeSync(); }); }, 50);
        setTimeout(function(){ vc5641Call('v52RefreshBusinessDayUI', function(){ return typeof v52RefreshBusinessDayUI === 'function' && v52RefreshBusinessDayUI(); }); }, 250);
        setTimeout(function(){ vc5641Call('forceUpdateInsightsNumbersFromTransactions', function(){ return typeof forceUpdateInsightsNumbersFromTransactions === 'function' && forceUpdateInsightsNumbersFromTransactions(); }); }, 450);
        setTimeout(function(){ vc5641Call('renderBusinessCalendar', function(){ return typeof renderBusinessCalendar === 'function' && renderBusinessCalendar(); }); }, 650);
    }

    const vc5641PreviousOnload = window.onload;
    window.onload = function(event) {
        vc5641ShowPosFallback();
        vc5641Call('previous window.onload', function(){
            if (typeof vc5641PreviousOnload === 'function') return vc5641PreviousOnload.call(window, event);
        });
        vc5641FullBoot();
    };
    ...
    setTimeout(vc5641LightRender, 1000);
    setTimeout(vc5641FullBoot, 2500);
})();


// v5.6.45: after POS is visible, allow delayed Tailwind load if present.
(function(){
  if (window.__vc5644KickTailwindAfterBoot) return;
  window.__vc5644KickTailwindAfterBoot = true;
  setTimeout(function(){
    try {
      if (typeof window.__vc5644LoadTailwindLater === 'function') window.__vc5644LoadTailwindLater();
    } catch (e) {
      console.warn('Tailwind delayed load skipped', e);
    }
  }, 5000);
})();
